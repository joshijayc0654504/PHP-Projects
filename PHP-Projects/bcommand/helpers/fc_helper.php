<?php
	...
	$res = '<select name="currency_select"  onChange="document.forms['."'{$formname}'".'].submit();">';
		...
		$menu .= "</li>\n";
            ...
            elseif($type=='|')
            {
                if($page==$row->menuitem)
                    ...
                    $cls ='';
                $menu .= ' <a href="'.$row->menulink.'"'.$cls.'>'.$row->menuitem.'</a> |';
            }
	    else
	    {
                if($page==$row->menuitem)
                    ...
                    $cls ='';
                $menu .= ' <a href="'.$row->menulink.'"'.$cls.'>'.$row->menuitem.'</a> <br/>';		
	    }
        ...
        {	    
            if($type=='li')
            {
                ...
                if($submenu==1 && $row->subitems>0)
                {
                    ...
                    foreach($query->result() as $line)
                    {
                        $menu .= "\n".'<li><a href="'.$line->menulink.'">'.$line->menuitem."</a></li>\n";
                    }
                    $menu .= "\n</ul>";
                }
    ...
    function get_home_sliders()
    {
	    ...
	    $slides .= '<div class="slide-item"><img src="'.img_format('productres/'.$row->product_picture, 'thumb').'"/>';
	    $slides .= '<span class="pname">'.rightLang($row->product_name,$row->product_name_fr).'</span>';
	    $slides .= '<span class="buynow"><a href="/'.$row->url.'">'.lang('Buy Now').'</a></span>';
    ...
    function get_coupon_discount($code,$amount)
    {
        ...
        $ci->db->where('discount_type','certificate');
...
function showCalendar($method=0,$days=30)
{
    echo draw_calendar(time(),time() + 60*60*24*$days,$method);
}
...
function draw_calendar($start,$finish,$method=0){
	...
	for($j=0; $j<= $months; $j++) {
	    ...
	    $calendar .= '<h2>'.date('F Y',mktime(0,0,0,$month,1,$year)).'</h2>';
