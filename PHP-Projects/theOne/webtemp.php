    ...
    if(isset($_POST['submit']) && $_POST['submit'] == 'Search'){
        ...
        if(isset($_POST['catname'])){
            ...
            $join .= ' JOIN category c ON c.catid=webtemp.catid';
            $where .= 'webtemp.catid IN ('.$catname.') AND ';
        }
        if(isset($_POST['scatname'])){
            ...
            $join .= ' JOIN subcategory sc ON sc.scatid=webtemp.scatid';
            $where .= 'webtemp.scatid IN ('.$scatname.') AND ';
        }
        ...
        $sql = "select webtemp.photo".$table." from webtemp ".$join." where ".$where; //catid in ($catname) AND scatid in ($scatname) AND keyword like '%$keyword%'";
        //echo $sql;
    ...
    <input type="hidden" name="webcart"  value="" />
    ...
    $limit = 12;
    $queryTotal = mysql_query('select webid from webtemp') or die(mysql_error());
    // Get total products
    ...
    if(isset($_REQUEST['page']) && $_REQUEST['page'] != ''){
        ...
        if($page!=1){
                $active = $page;
                $query = mysql_query('SELECT w.* ,c.catname,s.scatname FROM webtemp w JOIN category c ON w.catid = c.catid JOIN subcategory s ON w.scatid = s.scatid ORDER BY webid DESC LIMIT '.$limit*($page-1).','.$limit) or die(mysql_error());
        }else{$query = mysql_query('SELECT b.* ,c.catname,s.scatname FROM webtemp w JOIN category c ON w.catid = c.catid JOIN subcategory s ON w.scatid = s.scatid ORDER BY webid DESC LIMIT 0,'.$limit) or die(mysql_error());}
        
        }
        else{
            $query = mysql_query('SELECT w.* ,c.catname,s.scatname FROM webtemp w JOIN category c ON w.catid = c.catid JOIN subcategory s ON w.scatid = s.scatid ORDER BY webid DESC LIMIT 0,'.$limit) or die(mysql_error());
    
        }?>
        
    <!--pagination ends-->
    <?php
    /*$sql = "SELECT w.* ,c.catname,s.scatname FROM webtemp w JOIN category c ON w.catid = c.catid JOIN subcategory s ON w.scatid = s.scatid";
    ...
    ?>
     <form action="cart.php" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="webid" value="<?php echo $data['webid']; ?>" />
    ...
    <!--<img src="admin/images/<?php echo $data['photo'];?>" height="75"  width="100"
            ...
            <div class="image-row">
                <div class="image-set">
                    <a class="example-image-link" href="admin/images/<?php echo $data['photo'];?>" data-lightbox="example-set" title="<?= $data['photo'];?>"><img class="example-image" src="admin/images/<?php echo $data['photo'];?>" alt="no photo" width="160" height="120"/></a><br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="hidden" name="webcart"  value="" />
                <input type="submit" name="buy" value="Add to cart"  />
