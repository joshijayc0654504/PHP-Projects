    ...
    if(isset($_POST['submit']) && $_POST['submit'] == 'Search'){
        ...
        if(isset($_POST['catname'])){
            ...
            $join .= ' JOIN category c ON c.catid=businesscard.catid';
            $where .= 'businesscard.catid IN ('.$catname.') AND ';
        }
        if(isset($_POST['scatname'])){
            ...
            $join .= ' JOIN subcategory sc ON sc.scatid=businesscard.scatid';
            $where .= 'businesscard.scatid IN ('.$scatname.') AND ';
        }
        ...
        $sql = "select businesscard.photo".$table." from businesscard ".$join." where ".$where; //catid in ($catname) AND scatid in ($scatname) AND keyword like '%$keyword%'";
        //echo $sql;
        ...
        <input type="hidden" name="buid" value="<?php echo $data['buid']; ?>" />
        ...
        <div class="image-row">
                <div class="image-set">
                    ...
                    Price:<?php echo $data['price'];?>
                     <input type="hidden" name="businesscart"  value="" />
    ...
    $limit = 12;
    $queryTotal = mysql_query('select buid from businesscard') or die(mysql_error());
    // Get total products
    ...
    if(isset($_REQUEST['page']) && $_REQUEST['page'] != ''){
        ...
        if($page!=1){
                $active = $page;
                $query = mysql_query('SELECT b.* ,c.catname,s.scatname FROM businesscard b JOIN category c ON b.catid = c.catid JOIN subcategory s ON b.scatid = s.scatid ORDER BY buid DESC LIMIT '.$limit*($page-1).','.$limit) or die(mysql_error());
        }else{$query = mysql_query('SELECT b.* ,c.catname,s.scatname FROM businesscard b JOIN category c ON b.catid = c.catid JOIN subcategory s ON b.scatid = s.scatid ORDER BY buid DESC LIMIT 0,'.$limit) or die(mysql_error());}
        
        }
        else{
            $query = mysql_query('SELECT b.* ,c.catname,s.scatname FROM businesscard b JOIN category c ON b.catid = c.catid JOIN subcategory s ON b.scatid = s.scatid ORDER BY buid DESC LIMIT 0,'.$limit) or die(mysql_error());
    ...
    <?php
        /*$sql = "SELECT b.* ,c.catname,s.scatname FROM businesscard b JOIN category c ON b.catid = c.catid JOIN subcategory s ON b.scatid = s.scatid";
